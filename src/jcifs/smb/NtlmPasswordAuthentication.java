...
public final class NtlmPasswordAuthentication implements Principal,
	...
	static final String DEFAULT_DOMAIN = Config.getProperty(
			"jcifs.smb.client.domain", "?");

	private static final String DEFAULT_USERNAME = Config.getProperty(
			"jcifs.smb.client.username", "GUEST");

	static final String BLANK = "";
	static final String DEFAULT_PASSWORD = Config.getProperty(
	...
	static public byte[] getPreNTLMResponse(String password, byte[] challenge) {
		...
		try {
			passwordBytes = password
					.toUpperCase()
					.getBytes(
							Config.getProperty("jcifs.encoding", System
									.getProperty("file.encoding"))/* ServerMessageBlock.OEM_ENCODING */);
	...
	static final NtlmPasswordAuthentication NULL = new NtlmPasswordAuthentication(
			"", "", "");
	static final NtlmPasswordAuthentication GUEST = new NtlmPasswordAuthentication(
			"?", "GUEST", "");
	static final NtlmPasswordAuthentication DEFAULT = new NtlmPasswordAuthentication(
			null);
	...
	public NtlmPasswordAuthentication(String domain, String username,
		...
		if (domain == null || username == null || ansiHash == null
				|| unicodeHash == null) {
			throw new IllegalArgumentException(
					"External credentials cannot be null");
		}
		this.domain = domain;
		this.username = username;
		this.password = null;
	...
	public byte[] getUnicodeHash(byte[] challenge) {
		...
		case 5:
			/*
			 * if( clientChallenge == null ) { clientChallenge = new byte[8];
			 * RANDOM.nextBytes( clientChallenge ); } return
			 * getNTLMv2Response(domain, username, password, null, challenge,
			 * clientChallenge);
			 */
			return new byte[0];
		default:
	 ...
	 * @param challenge
	 *            The server challenge.
	 * @return A <code>byte[]</code> containing the effective user session
	 *         key, used in SMB MAC signing and NTLMSSP signing and sealing.
	 */
	...
	void getUserSessionKey(byte[] challenge, byte[] dest, int offset)
		...
		case 5:
			...
			HMACT64 hmac = new HMACT64(md4.digest());
			hmac.update(username.toUpperCase()
	...
	public boolean equals(Object obj) {
		if (obj instanceof NtlmPasswordAuthentication) {
			...
			if (ntlm.domain.toUpperCase().equals(domain.toUpperCase())
					&& ntlm.username.toUpperCase().equals(
